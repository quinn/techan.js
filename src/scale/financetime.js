...
export const financetime = function(d3_scale_linear, d3_time, d3_bisect, techan_util_rebindCallback, scale_widen, techan_scale_zoomable) {  // Injected dependencies
	...
	var intradayFormat = d3_v3_multi_shim([
		...
		[d3_time.timeFormat(TIME_FORMAT), () => true]
	])
	...
	var genericFormat = [
		...
		d3_v3_multi_shim([
			...
			[d3_time.timeFormat('%I:%M'),     d  => d.getMinutes()],
			[d3_time.timeFormat(TIME_FORMAT), d  => d.getHours()],
	...
	var intradayFormatUtc = d3_v3_multi_shim([
		...
		[d3_time.utcFormat(TIME_FORMAT), () => true]
	])

	var genericFormatUtc = [d3_time.timeSecond, 1, d3_v3_multi_shim([
			[d3_time.utcFormat(':%S'),       d  => d.getUTCSeconds()],
			[d3_time.utcFormat('%I:%M'),     d  => d.getUTCMinutes()],
			[d3_time.utcFormat(TIME_FORMAT), d  => d.getUTCHours()],
	...
	var techan_scale_financetime = () =>
		financetime({ daily: dailyTickMethod, intraday: intradayTickMethod }, genericFormat)

	techan_scale_financetime.utc = () =>
		financetime({ daily: dailyTickMethodUtc, intraday: intradayTickMethodUtc }, genericFormatUtc)
